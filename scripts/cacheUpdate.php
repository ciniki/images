...
foreach($images as $iid => $image) {
    $ciniki['tenant']['settings']['cache_dir'] = $ciniki['config']['ciniki.core']['cache_dir'] . '/'
        . $image['tenant_uuid'][0] . '/' . $image['tenant_uuid'];

    $cache_dir = $ciniki['tenant']['settings']['cache_dir'] . '/' 
    ...
    if( !file_exists("$cache_dir/t75.jpg")
        ...
        ciniki_images_loadCacheThumbnail($ciniki, $image['tnid'], $image['image_id'], 75);
    }   
    //
    ...
    if( !file_exists("$cache_dir/t300.jpg")
        ...
        ciniki_images_loadCacheThumbnail($ciniki, $image['tnid'], $image['image_id'], 300);
    }
...
//  }
    ...
    if( !file_exists("$cache_dir/o2000_2000.jpg")
        ...
        ciniki_images_loadCacheOriginal($ciniki, $image['tnid'], $image['image_id'], 2000, 2000);
    }
