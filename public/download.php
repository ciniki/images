...
// Description
// -----------
// This function will return the image in the original format.
//
// Arguments
// ---------
// api_key:
...
// Binary image data
//
function ciniki_images_download($ciniki) {
    //
    // Check args
    //
    ...
    if( $args['version'] == 'thumbnail' ) {
        ...
        ciniki_core_loadMethod($ciniki, 'ciniki', 'images', 'private', 'loadOriginal');
        $rc = ciniki_images_loadOriginal($ciniki, $args['tnid'], $args['image_id']);
    }
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
